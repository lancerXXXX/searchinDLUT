...
<div class="container" role="main">
    ...
    <c:set var="updateUrl" value="${requestScope.updateurl}"/>
            ...
            <table class="table table-hover table-striped table-bordered table-sm" border="1">
                ...
                <c:forEach var="user" items="${informations}">
                    <tr>
                        ...
                        <td>${user.getName()}</td>
                        <td>${user.getEmail()}</td>
                        <td><a href="${pageContext.request.contextPath}/login/update_servlet?telephone=${user.getTelephone()}&student_id=${user.getStudentId()}&teacher_id=${user.getTeacherId()}&qq=${user.getQq()}&name=${user.getName()}&email=${user.getEmail()}">更该数据</a></td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
